...
# ------------------------------------------------- the estimate's stated basis

@pytest.mark.parametrize("legs, expected", [
    ([{"broker": "Public", "accounts_low": 21, "accounts_high": 21}], "21 accounts"),
    ([{"broker": "Public", "accounts_low": 21, "accounts_high": 23}], "21+ accounts"),
    ([{"broker": "Public", "accounts_low": 20, "accounts_high": 20},
      {"broker": "Robinhood", "accounts_low": 3, "accounts_high": 3}], "23 accounts"),
    ([{"broker": "Public", "accounts_low": 1, "accounts_high": 1}], "1 account"),
])
def test_the_account_basis_is_spelled_out(legs, expected):
    assert A._sell_accounts_label({"legs": legs}) == expected


def test_an_older_record_falls_back_to_its_total():
    assert A._sell_accounts_label({"accounts": 12}) == "12 accounts"


def test_no_count_means_no_claim():
    assert A._sell_accounts_label({"legs": []}) == ""
    assert A._sell_accounts_label({"legs": [{"broker": "Public"}]}) == ""


def test_broker_names_no_longer_carry_the_count():
    """They sat beside our own coverage numbers and invited being read as ours."""
    sell = {"legs": [{"broker": "Public", "accounts_low": 21, "accounts_high": 21},
                     {"broker": "Robinhood", "accounts_low": 3, "accounts_high": 3}]}
    text = A._sell_legs_text(sell)
    assert text == "Public  ·  Robinhood"
    assert "21" not in text


def test_a_repeated_brokerage_is_listed_once():
    sell = {"legs": [{"broker": "Public", "accounts_low": 7},
                     {"broker": "Public", "accounts_low": 5}]}
    assert A._sell_legs_text(sell) == "Public"
